...
class Report(Base):
    ...
    # --- Grounding (Phase 10) --------------------------------------------
    # How many findings this report could not source, and how many citations
    # the model fabricated. Nullable and null for anything not generated by
    # this app — same rule as the cost columns: "not measured" is not zero.
    ungrounded_findings: Mapped[int | None] = mapped_column(Integer)
...
class FindingEvidence(Base):
    """One resolved citation: this finding came from that log content.

    **One table for all five sections, decided deliberately.** Per-section
    evidence tables would mean five migrations for one concept, five joins to
    render a report, and five queries to compute a grounding rate — and the
    Phase 11 eval harness wants that rate as a single number across everything.
    The cost is that `item_id` cannot carry a foreign key, because it points
    into one of five tables; referential integrity comes from the cascade on
    `report_id` instead, so evidence dies with its report either way.

    `chunk_id` is the document-wide chunk index — stable across re-ingest
    because the splitter is deterministic, which is what makes it usable as a
    citation key at all. The row and line spans come from `chunk.py`, recorded
    at ingest because that is the only moment the mapping exists.

    `excerpt` is a copy, not a reference. The vector store is a separate volume
    from the database and the two can go out of step; a report that can no
    longer show what it was based on because Chroma was rebuilt would be a
    report whose evidence evaporated.

    A finding with **no row here is ungrounded** — flagged by absence rather
    than by a boolean, and never by deletion. See `services/grounding.py`.
    """

    __tablename__ = "finding_evidence"

    evidence_id: Mapped[uuid.UUID] = _pk()
    report_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("reports.report_id", ondelete="CASCADE"), nullable=False, index=True
    )
    # Which of the five section tables `item_id` refers to.
    ...
    # Provenance in the source file. Rows for tabular sources, lines for text —
    # null when the mapping could not be established exactly, never guessed.
    ...
    __table_args__ = (
        # The report view loads every finding's evidence in one query.
        Index("ix_finding_evidence_report_item", "report_id", "item_id"),
    )
